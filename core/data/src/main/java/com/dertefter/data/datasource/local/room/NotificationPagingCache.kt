...
class NotificationPagingCache(
    private val tab: String,
    private val localDataSource: LocalDataSource
) : CursorPersistentPagingCache<String, NotificationDto> {

    private val type = PageType.NOTIFICATION

    override suspend fun save(cursor: CursorBookmark<String>, state: CursorPageState<String, NotificationDto>) {
        if (state.isSuccessState()) {
            ...
            localDataSource.saveNotifications(type, tab, self, state.data)
        }
    }

    override suspend fun saveAll(entries: List<Pair<CursorBookmark<String>, CursorPageState<String, NotificationDto>>>) {
        entries.forEach { (cursor, state) -> save(cursor, state) }
    }

    override suspend fun load(self: String): Pair<CursorBookmark<String>, CursorPageState<String, NotificationDto>>? {
        val entity = localDataSource.getPage(type, tab, self) ?: return null
        return try {
            val data = localDataSource.getNotificationsForPage(type, tab, self)
    ...
    override suspend fun loadAll(): List<Pair<CursorBookmark<String>, CursorPageState<String, NotificationDto>>> =
    ...
    override suspend fun <R> transaction(block: suspend CursorPersistentPagingCache<String, NotificationDto>.() -> R): R {
        return block()
    }
}
